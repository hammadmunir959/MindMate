...
const ExperienceForm = ({ darkMode, data, onSave, onDirty, saving }) => {
  ...
  useEffect(() => {
    if (data) {
      setExperienceRecords(data.experience_records || data.experience || []);
    }
  }, [data]);
  ...
  const handleDelete = (recordId) => {
    if (window.confirm('Are you sure you want to delete this experience record?')) {
      setExperienceRecords(prev => prev.filter(r => (r.id || r) !== recordId));
      onDirty();
    }
  };
  ...
  const handleSaveRecord = (record) => {
    if (editingRecord) {
      ...
      setExperienceRecords(prev => [...prev, record]);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    onSave({ experience_records: experienceRecords });
  };
                    ...
                    <div className="flex-1">
                      ...
                      <p className={`text-sm mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        ...
                        {record.start_date && record.end_date && (
                          ` • ${new Date(record.start_date).getFullYear()} - ${new Date(record.end_date).getFullYear()}`
                        )}
                        {record.years && ` • ${record.years} years`}
                      </p>
                      {record.description && (
