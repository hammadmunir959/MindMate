...
class Settings(BaseSettings):
    ...
    EMAILS_FROM_NAME: str = "MindMate"

    @field_validator("SMTP_SERVER", mode="before")
    @classmethod
    def validate_smtp_server(cls, v: Any, info) -> str:
        # If SMTP_URL is in values (from env), use it
        if not v:
            return "smtp.gmail.com"
        # Strip http:// or https:// if provided in env as SMTP_URL
        if isinstance(v, str):
